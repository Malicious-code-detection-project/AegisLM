...
현재 단계는 README의 **Phase B: 최소 코드 뼈대 생성 완료 및 최초 push 준비**입니다.
...
3. [AGENTS.md](AGENTS.md) - 협업 운영 규칙과 PR 기준
4. [CONTRIBUTING.md](CONTRIBUTING.md) - 팀원 작업 가이드
...
Phase B의 기본 구조는 다음과 같습니다. 이 단계에서는 물리적 구조만 만들고, 실제 학습 스크립트와 config는 이후 단계에서 추가합니다.
...
│   └── schemas.py                    # JSON output contract, dataset record shape
...
현재 Phase B에서는 scaffold 검증이 중심입니다. 코드가 추가된 뒤에는 가능한 범위에서 다음 명령을 사용합니다.
